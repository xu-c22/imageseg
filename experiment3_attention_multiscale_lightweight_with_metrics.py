...
SAVE_DIR = r"U:\xucheng\image-seg\experiment3_attention_multiscale_lightweight_runs"
...
# =========================
# 模型模块：Attention + Multi-Scale + Lightweight
# =========================
...
class MultiScaleBottleneck(nn.Module):
    def __init__(self, in_ch):
        ...
        self.branch2 = DepthwiseSeparableConv(in_ch, branch_ch, kernel_size=3, padding=1, dilation=1)
        self.branch3 = DepthwiseSeparableConv(in_ch, branch_ch, kernel_size=3, padding=2, dilation=2)
        self.branch4 = DepthwiseSeparableConv(in_ch, branch_ch, kernel_size=3, padding=4, dilation=4)
        ...
        self.refine = LightResBlock(in_ch, in_ch)
...
class Down(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.block = LightResBlock(in_ch, out_ch)
...
class Up(nn.Module):
    def __init__(self, in_ch, skip_ch, out_ch):
        ...
        self.block = LightResBlock(in_ch + skip_ch, out_ch)
...
class UNetAttMSLight2D(nn.Module):
    def __init__(self, in_ch=1, out_ch=1, base_ch=32):
        super().__init__()
        self.inc = LightResBlock(in_ch, base_ch)
        ...
        self.ms = MultiScaleBottleneck(base_ch * 16)
...
def estimate_inference_time_ms(model, device, img_size=256, repeats=50, warmup=10):
    model.eval()
...
def boundary_f1_score(pred: np.ndarray, target: np.ndarray, tolerance=2, eps=1e-6):
    pb = mask_to_boundary(pred)
    gb = mask_to_boundary(target)
    ...
    dist_to_g = distance_transform_edt(1 - gb)
    dist_to_p = distance_transform_edt(1 - pb)

    p_match = ((pb == 1) & (dist_to_g <= tolerance)).sum()
    g_match = ((gb == 1) & (dist_to_p <= tolerance)).sum()

    p_count = pb.sum()
    g_count = gb.sum()

    precision = (p_match + eps) / (p_count + eps)
    recall = (g_match + eps) / (g_count + eps)
    bf1 = (2 * precision * recall + eps) / (precision + recall + eps)
    return float(bf1)


def hd95_and_assd(pred: np.ndarray, target: np.ndarray):
    pred = (pred > 0).astype(np.uint8)
    target = (target > 0).astype(np.uint8)
    ...
    all_d = np.concatenate([d_pg, d_gp], axis=0)
    ...
    for batch in loader:
        ...
        for thr in thresholds:
            ...
            for i in range(bs):
                ...
                hd95, assd = hd95_and_assd(pred_i, gt_i)
                bf1 = boundary_f1_score(pred_i, gt_i, tolerance=BOUNDARY_TOL)
...
@torch.no_grad()
def export_validation_predictions(model, loader, device, save_dir, threshold=0.5, postprocess=False, max_save=30):
    model.eval()
...
def main():
    ...
    model = UNetAttMSLight2D(in_ch=1, out_ch=1, base_ch=32).to(DEVICE)
    ...
    with open(log_txt, "w", encoding="utf-8") as f:
        f.write(
            ...
            "val_loss,val_dice,val_iou,val_precision,val_recall,val_hd95,val_assd,val_boundary_f1,val_best_thr,lr\n"
        )
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        msg = (
            ...
            f"HD95: {val_metrics['hd95']:.4f}, ASSD: {val_metrics['assd']:.4f}, BF1: {val_metrics['boundary_f1']:.4f}, "
        ...
        with open(log_txt, "a", encoding="utf-8") as f:
            f.write(
                ...
                f"{val_metrics['hd95']:.6f},{val_metrics['assd']:.6f},{val_metrics['boundary_f1']:.6f},"
